...
## 启用 outbox consumer dry-run launchd

`outbox_consumer.py` 可以作为 launchd 后台 dry-run 消费器启用。它只读取
`data/outbox_messages.jsonl` 中的 `pending` 记录，并把状态更新为
`sent_dry_run`；它不会真实发送微信、Telegram、Hermes push 或外部网络消息。
...
`sent_dry_run` 只表示已被本地 dry-run consumer 消费，不代表真实发送完成。
...
如果希望 Hermes 在 dry-run consumer 处理前查看 pending outbox，可调用：
